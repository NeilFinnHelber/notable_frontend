...
const [unlockIntent, setUnlockIntent] = useState<'view' | 'edit' | null>(null);
  ...
  // Ensure folders are loaded when modals open
  useEffect(() => {
    if (selectedNote && user?.sub && currentFolderId) {
      // Refresh folders when a note modal is opened
      getFolders(user.sub).then(allFolders => {
        // Only show folders that are direct children of the current folder
        const filteredFolders = allFolders.filter(folder => {
          const folderParentId = folder.folder_id !== null ? folder.folder_id : null;
          return folderParentId === currentFolderId;
        });
        
        // Store all folders in a separate variable for parent folder lookup
        window.allFolders = allFolders;
        
        // Update the folders state with only the relevant subfolders
        setFolders(filteredFolders);
        console.log('Filtered folders for note modal:', filteredFolders);
      }).catch(err => console.error('Error fetching folders:', err));
    }
  }, [selectedNote, user?.sub, currentFolderId]);
  
  useEffect(() => {
    if (selectedFolder && user?.sub && currentFolderId) {
      // Refresh folders when a folder modal is opened
      getFolders(user.sub).then(allFolders => {
        // Only show folders that are direct children of the current folder
        const filteredFolders = allFolders.filter(folder => {
          const folderParentId = folder.folder_id !== null ? folder.folder_id : null;
        ...
        // Update the folders state with only the relevant subfolders
        setFolders(filteredFolders);
        console.log('Filtered folders for folder modal:', filteredFolders);
      }).catch(err => console.error('Error fetching folders:', err));
    }
  }, [selectedFolder, user?.sub, currentFolderId]);
  ...
  // Load initial data and set folder ID based on URL only once
  ...
  // Create a ref to track if we're already loading data to prevent infinite loops
  const isLoadingDataRef = useRef(false);
  
  // When entering a folder, we'll load data once but not set up any polling


  // Always resolve folder from URL and load its content (even on reload)
  
  // Function to load folder data by name
  const loadFolderDataByName = async (folderName: string) => {
    ...
    try {
      // Safely extract user.sub and verify it exists
      const userSub = user?.sub;
      if (!userSub) {
        console.error('User sub is undefined in loadFolderDataByName');
        return;
      }
      
      console.log(`Loading folder data for folder name: ${folderName}`);
      
      // Get all notes and folders
      const allNotes = await getNotes(userSub);
      const allFolders = await getFolders(userSub);
      
      // Find the folder by name
      const folder = allFolders.find(f => f.name === folderName);
      
      if (folder) {
        console.log(`Found folder with name ${folderName}, ID: ${folder.id}`);
        
        // Update the current folder ID and related state
        setCurrentFolderId(folder.id);
        setCurrentFolderName(folder.name);
        setCurrentFolderType(folder.folder_type);
        
        // Get notes that belong to the current folder
        const folderNotes = allNotes.filter(note => note.folder_id === folder.id);
        // Get subfolders of the current folder
        const subFolders = allFolders.filter(f => f.folder_id === folder.id);
        
        console.log(`Loaded data: Found ${folderNotes.length} notes and ${subFolders.length} subfolders in folder ${folder.id}`);
        ...
        showToast({
          message: `Could not find folder "${folderName}"`,
          duration: 2000,
          color: 'danger'
        });
      ...
      console.error('Error loading folder data by name:', error);
      showToast({
        message: 'Failed to load folder data. Please try again.',
        duration: 2000,
        color: 'danger'
      });
    } finally {
      // Reset loading flag and hide loading indicator
  ...
  useIonViewWillEnter(() => {
    console.log('View entered, refreshing folder data');
    // Force a refresh of the data when the view is entered
    if (user?.sub) {
      // If we have a folder name in the URL, use that to load data
      if (decodedName) {
        console.log(`Loading folder data for name in URL: ${decodedName}`);
        // Reset loading flag to ensure we can load data
        isLoadingDataRef.current = false;
        loadFolderDataByName(decodedName);
      } else if (currentFolderId) {
        // We have a currentFolderId but no name in URL (direct navigation)
        console.log(`Loading folder data for currentFolderId: ${currentFolderId}`);
        // Reset loading flag to ensure we can load data
        isLoadingDataRef.current = false;
        
        // Load data for the current folder ID
        const loadFolderDataById = async () => {
          // Set loading flag to prevent concurrent calls
          isLoadingDataRef.current = true;
          ...
          try {
            ...
            const userSub = user?.sub;
            if (!userSub) {
              console.error('User sub is undefined in loadFolderDataById');
              return;
            }
            ...
            console.log(`Loaded data by ID: Found ${folderNotes.length} notes and ${subFolders.length} subfolders in folder ${currentFolderId}`);
            ...
            console.error('Error loading folder data by ID:', error);
            showToast({
              message: 'Failed to load folder data. Please try again.',
              duration: 2000,
              color: 'danger'
        ...
        // No folder specified, load root items
        ...
        const loadRootData = async () => {
          ...
          try {
            ...
            const userSub = user?.sub;
            if (!userSub) {
              console.error('User sub is undefined in loadRootData');
              return;
            }
            ...
            // If no specific folder (root level), show root items
            const rootNotes = allNotes.filter(note => Number(note.folder_id) === 0);
            // Filter out co-worker folders and special folders
            ...
            console.log(`Loaded root data: Found ${rootNotes.length} notes and ${rootFolders.length} folders`);
            
            // Sort notes and folders using the consistent sorting function
            rootNotes.sort(sortByYDescending);
            rootFolders.sort(sortByYDescending);
  ...
  // We use a ref to track if this is the first load or a genuine folder change
  const initialLoadRef = useRef(true);
  const prevFolderIdRef = useRef<string | null>(null);
  
  useEffect(() => {
    // Skip the initial render or when the component is just mounting
    ...
    // Only fetch data if the folder ID actually changed
    if (currentFolderId !== null && currentFolderId !== prevFolderIdRef.current) {
      console.log(`Folder ID changed from ${prevFolderIdRef.current} to ${currentFolderId}, refreshing data`);
      prevFolderIdRef.current = currentFolderId;
  ...
  const onAddFolder = () => {
    ...
    handleAddFolder(
      ...
      folderDescription,
      user,
      String(currentFolderId ?? 0), // parent folder ID is 0 for root level
      folderType,
      1,
      nextY,
      showToast,
  ...
  const onEditNote = (note: Note) => {
    ...
    setCalcNumberState(note.calc_number !== undefined ? note.calc_number : undefined);
    setModalTab('notes');
  };
    ...
    try {
      ...
      if (data.type === 'note') {
        ...
        if (noteToMove) {
          ...
          // Update state
          setNotes(prevNotes => 
            prevNotes.map(note => Number(note.id) === Number(noteId) ? updatedNote : note)
          );
  ...
  const onTurnNoteIntoFolder = (folderType: number) => {
    ...
    handleAddFolder(
      ...
      user,
      String(selectedNote.folder_id ?? 0), // Use the note's parent folder ID as string, default to "0"
      folderType,
      1,
  ...
  const handleReorderItems = async (event: CustomEvent) => {
    try {
      ...
      const rootNotes = notes.filter(note => Number(note.folder_id) === 0);
  ...
  // Keep track of navigation history to force data reload
  const [lastRoute, setLastRoute] = useState<string>('');

  // CRITICAL: Dedicated state variable to track if we need a refresh
  const [needsRefresh, setNeedsRefresh] = useState<boolean>(true);

 
  // Use Ionic's lifecycle hook to reload data EVERY time the view is entered
  // This runs BEFORE the component is visible to the user
  useIonViewWillEnter(() => {
    console.log('🔄 VIEW ENTERED: Loading folder content for', decodedName);
    
    // Reset state when entering view to avoid showing stale data
    setNotes([]);
    setFolders([]);
    setLoading(true);
    setNeedsRefresh(true); // Mark that we need a refresh
    
    // Force reload when navigating back to the same route from a different route
    // This ensures content loads even when URL doesn't change
    if (lastRoute !== decodedName) {
      console.log('Route changed from', lastRoute, 'to', decodedName);
      setLastRoute(decodedName);
    }
  });

  // This effect runs after the component has rendered, and will trigger IMMEDIATELY


  // Also reload when the URL parameter or user changes


  // Unified function to fetch folder content
  const fetchFolderContent = async () => {
    // Ensure user is authenticated before proceeding
    if (!user || !user.sub) {
      console.log('User not authenticated, cannot load folder data');
      setLoading(false);
      return;
    }
    
    // Store user.sub in a variable to ensure TypeScript knows it exists
    const userSub = user.sub;
    
    try {
      // Set loading state and clear existing data for a fresh start
      setLoading(true);
      console.log(`🔍 Fetching content for folder: "${decodedName}"`);
      
      // STEP 1: First get all folders to find the current folder by name
      const foldersResponse = await getFolders(userSub);
      
      // Log folder data for debugging
      console.log('Folders from API:', foldersResponse.map(f => 
        ({ id: f.id, name: f.name, type: f.folder_type, parent: f.folder_id })
      ));
      
      // Find the current folder by exact name match without restricting by folder_type
      const currentFolder = foldersResponse.find(folder => 
        folder.name === decodedName
      );
      
      // Handle case where folder isn't found
      if (!currentFolder) {
        console.error(`Could not find folder: "${decodedName}"`);
        setNotes([]);
        setFolders([]);
        setCurrentFolderId(null);
        setCurrentFolderName('');
        setCurrentFolderType(0);
        
        showToast({
          message: `Folder "${decodedName}" not found`,
          duration: 2000,
          color: 'danger'
        });
        setLoading(false);
      ...
      // We found the current folder - set folder metadata
      console.log('✓ FOUND CURRENT FOLDER:', currentFolder);
      const currentFolderId = currentFolder.id;
      
      // Update state with current folder info
      setCurrentFolderId(currentFolderId);
      setCurrentFolderName(currentFolder.name);
      setCurrentFolderType(currentFolder.folder_type);
      
      // STEP 2: Fetch notes directly using the folder ID parameter
      console.log(`⬇️ Loading notes directly for folder ID: ${currentFolderId}`);
      
      // Critical: Use Promise.all for better performance
      const [notesResponse, allUserProfiles] = await Promise.all([
        // Get notes for this specific folder only using the API's folderId parameter
        getNotes(userSub, String(currentFolderId)),
        // Ensure we have all user profiles for co-worker information
        Object.keys(userMap).length === 0 ? getUserCoWorkers(userSub) : Promise.resolve([])
      ]);
      
      // Update user map if needed
      if (allUserProfiles && allUserProfiles.length > 0) {
        const newUserMap = {...userMap};
        (allUserProfiles as unknown as CoWorkerDetail[]).forEach(profile => {
          // Only assign if profile is an object with user_id (CoWorkerDetail), not a string
          if (typeof profile === 'object' && profile !== null && 'user_id' in profile) {
            newUserMap[(profile as CoWorkerDetail).user_id] = profile as CoWorkerDetail;
          }
        });
        // Only update if we have new data
        if (Object.keys(newUserMap).length > Object.keys(userMap).length) {
          setUserMap(newUserMap);
        }
      }
      
      // Get only the subfolders for this specific folder
      const subfoldersResponse = foldersResponse.filter(folder => {
        const folderParentId = folder.folder_id !== null ? folder.folder_id : null;
        return folderParentId === currentFolderId;
      });
      
      console.log(`✓ Found ${notesResponse.length} notes and ${subfoldersResponse.length} subfolders`);
      
      // Sort by Y coordinate (highest first for proper display order)
      // This maintains the behavior where newest notes appear at the top
      notesResponse.sort(sortByYDescending);
      subfoldersResponse.sort(sortByYDescending);
      
      // CRITICAL: Force a state update for immediate rendering
      setNotes([...notesResponse]); // Use spread operator to ensure new array reference
      setFolders([...subfoldersResponse]);
      
      // Done loading
      setLoading(false);
      
    } catch (error) {
      console.error('Error in fetchFolderContent:', error);
      showToast({
        message: 'Failed to load folder data',
        duration: 2000,
        color: 'danger'
      });
      // Clear data and loading state on error
      setNotes([]);
      setFolders([]);
      setLoading(false);
    }
    ...
    <IonPage ref={page}>
      <IonHeader>
        <IonToolbar color={'primary'}>
          ...
          <IonTitle>{currentFolderName || 'Folder'}</IonTitle>
        </IonToolbar>
        <IonToolbar color={'primary'}>
        ...
        {activeTab === 'notes' ? (
          <>
            ...
            <IonReorderGroup disabled={false} onIonItemReorder={handleReorderItems}>
              ...
              {/* Display all folders and notes, applying minimal filtering in the render */}
{[
  ...folders.filter(folder => !folder.co_workers || folder.co_workers.length === 0), // Filter out folders with co_workers
  ...notes    // Use notes state which already contains filtered results
]
                .filter(item => {
                  ...
                  if (isFolder(item)) {
                    ...
                    return (
                      <IonItem key={`folder-${folder.id}`}>
                        {currentFolderId && (folders.find(f => f.id === currentFolderId)?.checklist) ? (
                          <input
                            type="checkbox"
                            checked={folder.crossed_out || false}
                            onChange={e => {
                              const updatedFolder = { ...folder, crossed_out: e.target.checked };
                              updateFolder(folder.id, updatedFolder, user?.sub || '');
                    ...
                    return (
                      <IonItemSliding key={`note-${note.id}`} ref={(el) => handleDrag(el, () => onDeleteNote(note.id))}>
                        {currentFolderId && (folders.find(f => f.id === currentFolderId)?.checklist) ? (
                          <input
                            type="checkbox"
                            checked={note.crossed_out || false}
                            onChange={e => {
                              const updatedNote = { ...note, crossed_out: e.target.checked };
                              updateNote(note.id, updatedNote, user?.sub || '');
      ...
      Your browser does not support the audio element.
        ...
        >
          ...
          <IonContent className="ion-padding">
            ...
            {modalTab === 'attachments' && selectedNote && (
  <>
    {/* Return to Parent Folder Button for Notes (Attachments Tab) */}
    <IonButton
      expand="block"
      color="medium"
      ...
      onClick={async () => {
        try {
          ...
          console.log('Attempting to return note to parent folder');
          console.log('Current note folder_id:', selectedNote.folder_id);
          ...
          if (user?.sub) {
            ...
            parentFolder = allFolders.find((f: Folder) => f.id === selectedNote.folder_id);
            console.log('Found parent folder:', parentFolder);
          } else {
            // Use the stored complete list if available
            parentFolder = window.allFolders ? 
              window.allFolders.find((f: Folder) => f.id === selectedNote.folder_id) : 
              folders.find((f: Folder) => f.id === selectedNote.folder_id);
          ...
          const updatedNote = { ...selectedNote, folder_id: String(newFolderId) };
...
{modalTab === 'settings' && (
        ...
        <IonModal 
          ...
          onDidDismiss={() => setShowAddNoteModal(false)}
        >
          <IonHeader>
            <IonToolbar>
              <IonTitle>Add Note</IonTitle>
              <IonButtons slot="end">
                <IonButton onClick={() => setShowAddNoteModal(false)}>
                  Close
                </IonButton>
              </IonButtons>
            </IonToolbar>
          ...
          <IonContent className="ion-padding">
            {folderModalTab === 'folder' && (
              <>
                ...
                {selectedFolder && (
                  <IonButton expand="block" color="danger" onClick={() => onDeleteFolder(Number(selectedFolder.id))}>
                    Delete Folder
                  </IonButton>
                )}
              </>
    ...
    <IonButton
      ...
      onClick={async () => {
        try {
          ...
          console.log('Current folder folder_id:', selectedFolder.folder_id);
          ...
          if (user?.sub) {
            ...
            // Find parent folder from the complete list
            parentFolder = allFolders.find((f: Folder) => f.id === selectedFolder.folder_id);
            console.log('Found parent folder:', parentFolder);
          } else {
            // Use the stored complete list if available
            parentFolder = window.allFolders ? 
              window.allFolders.find((f: Folder) => f.id === selectedFolder.folder_id) : 
              folders.find((f: Folder) => f.id === selectedFolder.folder_id);
            console.log('Using stored folders, parent folder:', parentFolder);
          }
          ...
          if (newFolderId === selectedFolder.id) {
            presentToast({
              message: 'Cannot move to prevent circular reference',
              duration: 2000,
              color: 'warning',
          ...
          const updatedFolder = { ...selectedFolder, folder_id: String(newFolderId) };
          await updateFolder(selectedFolder.id, updatedFolder, user?.sub || '');
                    ...
                    onIonChange={async (e) => {
                      ...
                      if (selectedFolder) {
                        ...
                        if (typeof window.refreshSharedFolders === 'function' && user?.sub) {
                          try {
                            await window.refreshSharedFolders(user.sub);
                            console.log('Refreshed shared folders after color change');
                          } catch (error) {
                            console.error('Error refreshing shared folders:', error);
                          }
                        }
