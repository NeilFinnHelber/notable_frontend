...
export const MindmapFolder: React.FC = () => {
  ...
  const [isClearConnectionMode, setIsClearConnectionMode] = useState(false);
...
useEffect(() => {
  const handler = () => {
    reloadDataAndConnections();
    connectionsInitialized.current = false; // Force rebuild
  };
  window.addEventListener('notable-reload-mindmap', handler);
  return () => window.removeEventListener('notable-reload-mindmap', handler);
}, []);

useEffect(() => {
  if (window.notableShouldReloadMindmap) {
    reloadDataAndConnections();
    connectionsInitialized.current = false;
    window.notableShouldReloadMindmap = false;
  }
  // ...existing code...
...
const reloadDataAndConnections = async () => {
  if (!user?.sub) return;
  const allNotes = await getNotes(user.sub);
  const allFolders = await getFolders(user.sub);
  setFolders(allFolders);
  ...
  // Hide minimap and overlays
  const minimap = minimapRef.current;
  if (minimap) minimap.style.visibility = "hidden";

  // Save original transform
  const originalTransform = canvasRef.current.style.transform;
  // Temporarily remove transform for screenshot
  canvasRef.current.style.transform = "";

  // Wait for the DOM to update
  await new Promise(res => setTimeout(res, 50));

  // Screenshot the full canvas DOM node
  await html2canvas(canvasRef.current, {
    ...
    const offsetX = canvasOffset.x * zoomFactor;
    const offsetY = canvasOffset.y * zoomFactor;
    const viewportWidth = window.innerWidth / zoomFactor;
    const viewportHeight = window.innerHeight / zoomFactor;

    // Calculate cropping coordinates in the screenshot
    if (!canvasRef.current) return;
    const cropX = (canvas.width / canvasRef.current.offsetWidth) * (-offsetX + (canvasRef.current.offsetWidth - viewportWidth) / 2);
    const cropY = (canvas.height / canvasRef.current.offsetHeight) * (-offsetY + (canvasRef.current.offsetHeight - viewportHeight) / 2);
    const cropWidth = canvas.width / zoomFactor;
    const cropHeight = canvas.height / zoomFactor;

    // Create a cropped canvas
    const croppedCanvas = document.createElement("canvas");
    croppedCanvas.width = window.innerWidth * window.devicePixelRatio;
    croppedCanvas.height = window.innerHeight * window.devicePixelRatio;
    ...
    // Restore transform and minimap on error
    canvasRef.current!.style.transform = originalTransform;
    if (minimap) minimap.style.visibility = "";
  });
};
...
const handleClearAttachmentField = async (field: 'image_url' | 'file_link' | 'voice_message_url') => {
 ...
 const drawConnections = () => {
  return connections.map((connection, index) => {
    ...
    const fromX = (fromObj.x || 0) + canvasOffset.x + 75;
    const fromY = (fromObj.y || 0) + canvasOffset.y + 20;
    ...
    return (
      <svg
        ...
        style={{
          ...
          width: '100%',
          height: '100%',
          pointerEvents: isClearConnectionMode ? 'auto' : 'none',
          zIndex: 1
        }}
        onClick={isClearConnectionMode ? (e) => {
          e.stopPropagation();
          handleClearSingleConnection(connection);
        } : undefined}
      >
        <line
          x1={fromX}
          ...
          x2={toX}
          y2={toY}
          stroke={isClearConnectionMode ? '#ff9800' : lineColor}
          strokeWidth={isClearConnectionMode ? "4" : "2"}
          style={{ cursor: isClearConnectionMode ? 'pointer' : 'default' }}
        />
      </svg>
    );
  });
...
const handleClearSingleConnection = async (connection: Connection) => {
  ...
  // Remove from backend (connected_to array)
  if (connection.from.type === 'note') {
    const note = notes.find(n => n.id === connection.from.id);
    if (note && Array.isArray(note.connected_to)) {
      const updatedConnectedTo = note.connected_to.filter(id => id !== connection.to.id);
      const updatedNote = { ...note, connected_to: updatedConnectedTo };
      await updateNote(note.id, updatedNote, user?.sub || '');
      setNotes(prev => prev.map(n => n.id === note.id ? updatedNote : n));
    }
  } else if (connection.from.type === 'folder') {
    const folder = folders.find(f => f.id === connection.from.id);
    if (folder && Array.isArray(folder.connected_to)) {
      const updatedConnectedTo = folder.connected_to.filter(id => id !== connection.to.id);
      const updatedFolder = { ...folder, connected_to: updatedConnectedTo };
...
const handleClearConnections = async () => {
  // Optionally, clear connections in backend as well
  // For now, just clear in frontend state
  setConnections([]);
  // Clear connected_to for all notes and folders in state and backend
  if (user?.sub) {
    // Clear notes
    for (const note of notes) {
      if (Array.isArray(note.connected_to) && note.connected_to.length > 0) {
        const updatedNote = { ...note, connected_to: [] };
        await updateNote(note.id.toString(), updatedNote, user.sub);
      }
    }
    // Clear folders
    for (const folder of folders) {
      if (Array.isArray(folder.connected_to) && folder.connected_to.length > 0) {
        const updatedFolder = { ...folder, connected_to: [] };
        await updateFolder(folder.id.toString(), updatedFolder, user.sub);
      }
    }
  }
  showToast({
      ...
      <IonHeader>
        <IonToolbar>
          ...
          <IonButtons slot="end">
            ...
            <IonButton color={isClearConnectionMode ? "warning" : "medium"} onClick={() => setIsClearConnectionMode(!isClearConnectionMode)}>
              {isClearConnectionMode ? "Exit Clear Connections" : "Clear Connections"}
            </IonButton>
      ...
      return (
        ...
        >
          ...
          {notes.map((note) => {
            ...
            // Helper to extract first percentage from note text
            const getPercentageFromText = (text?: string): number | null => {
              if (!text) return null;
            ...
            const percent = getPercentageFromText(note.text);
              ...
              <div style={{ 
                ...
                <div style={{ margin: 0, color: '#cccccc' }}>
                  {note.text?.includes('[') && note.text?.includes(']')
                    ...
                    : renderTextWithLinks(note.text || '', note.color)
                  }
                </div>

{/* Progress Bar if percentage is found */}
{percent !== null && (
  <div style={{ marginTop: 10 }}>
    <div style={{
      ...
      <div style={{
        width: `${percent}%`,
        height: '100%',
        background: percent === 100 ? 'var(--ion-color-success)' : 'linear-gradient(90deg, #4a9eff, #00e676)',
    ...
    <div style={{
      textAlign: 'right',
      fontSize: 12,
      color: percent === 100 ? 'var(--ion-color-success)' : '#aaa',
      marginTop: 2
    }}>
      {percent}% complete
    </div>
...
// Extend the Window interface to include 'notes', 'allFolders', and 'notableShouldReloadMindmap'
declare global {
  interface Window {
    notes?: any[];
    allFolders?: any[];
    refreshSharedFoldersSidebar?: () => void;
    notableShouldReloadMindmap?: boolean;
  }
}
...
// This function must be inside the MindmapFolder component, after all useState declarations
        ...
        return (
          <React.Fragment key={lineIdx}>
            {parts.map((part, i) => {
              ...
              // Replace tab characters with 4 non-breaking spaces for visual alignment
              const withTabs = part.split('\t').reduce((acc, curr, idx) => {
                if (idx === 0) return [curr];
                return [
                  ...acc,
                  <span key={`tab-${lineIdx}-${i}-${idx}`} style={{ whiteSpace: 'pre' }}>{'\u00A0\u00A0\u00A0\u00A0'}</span>,
                  curr
                ];
              }, [] as (string | JSX.Element)[]);
              return <React.Fragment key={i}>{withTabs}</React.Fragment>;
            })}
